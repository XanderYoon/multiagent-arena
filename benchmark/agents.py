...
class LLMRuntime:
    def __init__(self, model_config, prompt_config, call_log_path, prompt_family=None, append=False):
        self.model_config = model_config
        self.prompt_manager = PromptManager(prompt_config, selected_family=prompt_family)
        self.call_logger = JSONLCallLogger(call_log_path, append=append)
        ...
        self.call_index = 0
        if append and Path(call_log_path).exists():
            with Path(call_log_path).open("r", encoding="utf-8") as handle:
                self.call_index = sum(1 for _ in handle)
...
def configure_runtime(model_config, prompt_config, call_log_path, prompt_family=None, alias="default", reset=False, append=False):
...
def configure_runtime_registry(model_configs, prompt_config, call_log_dir, prompt_family=None, default_alias=None, append=False):
    global _RUNTIMES, _DEFAULT_RUNTIME_ALIAS
    _RUNTIMES = {}
    call_log_dir = Path(call_log_dir)
    call_log_dir.mkdir(parents=True, exist_ok=True)
    resolved_default_alias = default_alias or next(iter(model_configs.keys()))
    for alias, model_config in model_configs.items():
        if len(model_configs) == 1 and alias == resolved_default_alias:
            log_path = call_log_dir / "llm_calls.jsonl"
        else:
            log_path = call_log_dir / f"llm_calls_{alias}.jsonl"
        _RUNTIMES[alias] = LLMRuntime(
            model_config,
            prompt_config,
            log_path,
    ...
    _DEFAULT_RUNTIME_ALIAS = resolved_default_alias
