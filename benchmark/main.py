...
from benchmark.loader import load_benchmark_bundle
...
from benchmark.architectures import build_architecture_registry, run_architecture
from benchmark.agents import configure_runtime_registry
...
class ProgressTracker:
    def __init__(self, trial_schedule):
        self.total = len(trial_schedule["schedule"])
        self.completed = 0
        self.phase_totals = {}
        self.phase_completed = {}
        for entry in trial_schedule["schedule"]:
            phase = entry["phase"]
            self.phase_totals[phase] = self.phase_totals.get(phase, 0) + 1
        for phase in self.phase_totals:
            self.phase_completed[phase] = 0

    def start(self, logger, entry):
        phase = entry["phase"]
        logger.log(
            f"[PROGRESS] Starting {self.completed + 1}/{self.total} | "
            f"{phase} {self.phase_completed[phase] + 1}/{self.phase_totals[phase]} | "
            f"game={entry['game']} | trial={entry['trial']} | participants={','.join(entry['participants'])}"
        )

    def finish(self, logger, entry):
        phase = entry["phase"]
        self.completed += 1
        self.phase_completed[phase] += 1
        overall_pct = (self.completed / self.total) * 100 if self.total else 100.0
        phase_pct = (
            (self.phase_completed[phase] / self.phase_totals[phase]) * 100
            if self.phase_totals[phase]
            else 100.0
        )
        logger.log(
            f"[PROGRESS] Finished {self.completed}/{self.total} ({overall_pct:.1f}%) | "
            f"{phase} {self.phase_completed[phase]}/{self.phase_totals[phase]} ({phase_pct:.1f}%)"
        )
...
def main():
    ...
    progress = ProgressTracker(trial_schedule)
    model_identity = {
